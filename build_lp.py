...
2026-09-03 精简：废弃分编单文件（存在论/本质论/概念论/先行概念 .html 不再生成），
只产出全书合并版 笔记/小逻辑_注释版.html：
  python build_lp.py          → 全书合并版
  python build_lp.py all      → 同上（兼容旧用法）
"""
import json
import re
import sys
...
# 编定义（数据配置：notes 文件 + § 范围 + 章节代号）；不产分编文件
PARTS = {
    "vorbegriff": {"notes": "vorbegriff.json", "chapter": 1, "lo": 1, "hi": 83},
    "sein":       {"notes": "sein.json",       "chapter": 2, "lo": 84, "hi": 111},
    "essence":    {"notes": "essence.json",    "chapter": 3, "lo": 112, "hi": 159},
    "begriff":    {"notes": "begriff.json",    "chapter": 4, "lo": 160, "hi": 244},
}

# 合体顺序：按书序（先行概念 → 存在论 → 本质论 → 概念论）
ALL_ORDER = ["vorbegriff", "sein", "essence", "begriff"]

OUT_ALL = "小逻辑_注释版.html"
...
if __name__ == "__main__":
    # 只产全书合并版（2026-09-03：分编废弃）；all/无参均走合并
    build_all()
